...
<head>
    ...
    <style>
        ...
        .siniestro-card .card-header .numero {
            font-weight: 600;
            font-size: 1.1rem;
            color: var(--primary-color);
        }
        
        .siniestro-card .card-content {
            margin-bottom: 0.75rem;
        }
        
        .siniestro-card .card-content .info-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.5rem;
        }
        
        .siniestro-card .card-content .info-row .label {
            font-weight: 500;
            color: #666;
        }
        
        .siniestro-card .card-content .info-row .value {
            font-weight: 400;
            text-align: right;
        }
        
        .siniestro-card .card-footer {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-top: 0.75rem;
        ...
        .siniestro-card .card-actions {
            display: flex;
            gap: 8px;
        }
        
        .siniestro-card .card-actions a, 
        .siniestro-card .card-actions button {
            color: var(--primary-color);
            transition: var(--transition);
            text-decoration: none;
            display: inline-flex;
        ...
        .siniestro-card .card-actions a.edit-icon, 
        .siniestro-card .card-actions button.edit-icon {
            color: #0d6efd; /* Azul para editar */
        }
        
        .siniestro-card .card-actions a.view-icon, 
        .siniestro-card .card-actions button.view-icon {
            color: #198754; /* Verde para ver */
        }
        
        .siniestro-card .card-actions a.delete-icon, 
        .siniestro-card .card-actions button.delete-icon {
            color: #dc3545; /* Rojo para eliminar */
        }
        
        .siniestro-card .card-actions a:hover, 
        .siniestro-card .card-actions button:hover {
            background-color: rgba(79, 140, 255, 0.1);
        }
        
        .siniestro-card .card-actions a.edit-icon:hover, 
        .siniestro-card .card-actions button.edit-icon:hover {
            background-color: rgba(13, 110, 253, 0.1);
        }
        
        .siniestro-card .card-actions a.view-icon:hover, 
        .siniestro-card .card-actions button.view-icon:hover {
            background-color: rgba(25, 135, 84, 0.1);
        }
        
        .siniestro-card .card-actions a.delete-icon:hover, 
        .siniestro-card .card-actions button.delete-icon:hover {
            background-color: rgba(220, 53, 69, 0.1);
        }
        
        /* Animación de ripple para los botones de acción */
        .action-icons a, .action-icons button,
        .siniestro-card .card-actions a, .siniestro-card .card-actions button {
            position: relative;
            overflow: hidden;
        }
        
        .action-icons a::after, .action-icons button::after,
        .siniestro-card .card-actions a::after, .siniestro-card .card-actions button::after {
            content: '';
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .siniestro-card .card-actions a:focus:not(:active)::after, .siniestro-card .card-actions button:focus:not(:active)::after {
        ...
        /* Estilo para el icono de vehículo en la tabla vacía */
        .empty-table-icon {
            font-size: 3rem;
            color: var(--primary-color);
            margin-bottom: 1rem;
...
<body>
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 mb-md-4">
                            <h2 class="card-title mb-3 mb-md-0">
                                <i class="bi bi-car-front-fill me-2 d-none d-sm-inline-block"></i>Gestión de Siniestros
                            </h2>
                        </div>
                        ...
                        @if(request('search') || request('estado'))
                            <div class="alert alert-info mb-3 mb-md-4">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <a href="{{ route('siniestros.index') }}" class="btn btn-sm btn-outline-secondary">
                        ...
                        <!-- Tabla de siniestros (vista desktop y tablet) -->
                                    ...
                                    @forelse($siniestros as $siniestro)
                                        <tr>
                                            <td class="action-icons">
                                                <a href="{{ route('siniestros.edit', $siniestro) }}" title="Editar" class="edit-icon">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <a href="{{ route('siniestros.show', $siniestro) }}" title="Ver detalles" class="view-icon">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                                <button type="button" title="Eliminar" class="delete-icon" 
                                                    ...
                                                    data-numero="{{ $siniestro->numero }}"
                                                    data-vehiculo="{{ $siniestro->vehiculo->marca }} {{ $siniestro->vehiculo->modelo }}"
                                                    data-matricula="{{ $siniestro->vehiculo->matricula }}"
                                                    data-cliente="{{ $siniestro->cliente->nombre }} {{ $siniestro->cliente->apellidos }}"
                                            ...
                                            <td>
                                                <span class="estado-badge" style="background-color: {{ $siniestro->getEstadoColor() }};">
                                                    {{ $siniestro->estado }}
                                                </span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="8" class="text-center py-4">
                                                ...
                                                @if(request('search') || request('estado'))
                                                    No se encontraron siniestros que coincidan con los criterios de búsqueda.
                                                @else
                        ...
                        <div class="mobile-cards">
                            @forelse($siniestros as $siniestro)
                                <div class="siniestro-card">
                                    <div class="card-header">
                                        <div class="numero">Siniestro #{{ $siniestro->numero }}</div>
                                        <span class="estado-badge" style="background-color: {{ $siniestro->getEstadoColor() }};">
                                            {{ $siniestro->estado }}
                                        </span>
                                    </div>
                                    <div class="card-content">
                                        <div class="info-row">
                                            ...
                                            <div class="value">{{ $siniestro->vehiculo->marca }} {{ $siniestro->vehiculo->modelo }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Matrícula:</div>
                                            <div class="value">{{ $siniestro->vehiculo->matricula }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Cliente:</div>
                                            <div class="value">{{ $siniestro->cliente->nombre }} {{ $siniestro->cliente->apellidos }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Entrada:</div>
                                            <div class="value">{{ $siniestro->fecha_entrada->format('d/m/Y') }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Salida:</div>
                                            <div class="value">{{ $siniestro->fecha_salida ? $siniestro->fecha_salida->format('d/m/Y') : 'Pendiente' }}</div>
                                        </div>
                                    </div>
                            ...
                            @empty
                                <!-- Se eliminó el mensaje y el icono que estaba por fuera de la tabla -->
                            @endforelse
                        </div>
                        ...
                        @if($siniestros->hasPages())
                            <div class="d-flex justify-content-center mt-3 mt-md-4">
                                {{ $siniestros->appends(['search' => request('search'), 'estado' => request('estado')])->links() }}
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <a href="{{ route('siniestros.create') }}" class="floating-action-button">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>¿Estás seguro de que deseas eliminar el siniestro <span class="siniestro-info" id="siniestroNumero"></span>?</p>
                    <p class="mb-1">Vehículo: <span class="siniestro-info" id="siniestroVehiculo"></span></p>
                    <p class="mb-1">Matrícula: <span class="siniestro-info" id="siniestroMatricula"></span></p>
                    <p>Cliente: <span class="siniestro-info" id="siniestroCliente"></span></p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (deleteModal) {
                deleteModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    document.getElementById('siniestroNumero').textContent = '#' + numero;
                    document.getElementById('siniestroVehiculo').textContent = vehiculo;
                    document.getElementById('siniestroMatricula').textContent = matricula;
                    document.getElementById('siniestroCliente').textContent = cliente;
                    
                    // Actualizar el formulario de eliminación
                    const deleteForm = document.getElementById('deleteForm');
                    if (deleteForm) {
                        deleteForm.action = `{{ route('siniestros.destroy', '') }}/${id}`;
                    }
                });
