...
<head>
    ...
    <title>{{ config('app.name', 'MiraCar') }} - Documentación</title>
    ...
    <style>
        ...
        .document-card .card-header .nombre {
            font-weight: 600;
            font-size: 1.1rem;
            color: var(--primary-color);
        }
        ...
        .document-card .card-content .info-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.5rem;
        }
        
        .document-card .card-content .info-row .label {
            font-weight: 500;
            color: #666;
        }
        
        .document-card .card-content .info-row .value {
            font-weight: 400;
            text-align: right;
        }
        ...
        .document-card .card-actions a.edit-icon, 
        .document-card .card-actions button.edit-icon {
            color: #0d6efd; /* Azul para editar */
        }
        
        .document-card .card-actions a.view-icon, 
        .document-card .card-actions button.view-icon {
            color: #198754; /* Verde para ver */
        }
        
        .document-card .card-actions a.delete-icon, 
        .document-card .card-actions button.delete-icon {
            color: #dc3545; /* Rojo para eliminar */
        }
        
        .document-card .card-actions a:hover, 
        .document-card .card-actions button:hover {
            background-color: rgba(79, 140, 255, 0.1);
        }
        
        .document-card .card-actions a.edit-icon:hover, 
        .document-card .card-actions button.edit-icon:hover {
            background-color: rgba(13, 110, 253, 0.1);
        }
        
        .document-card .card-actions a.view-icon:hover, 
        .document-card .card-actions button.view-icon:hover {
            background-color: rgba(25, 135, 84, 0.1);
        }
        
        .document-card .card-actions a.delete-icon:hover, 
        .document-card .card-actions button.delete-icon:hover {
            background-color: rgba(220, 53, 69, 0.1);
        }
        
        /* Animación de ripple para los botones de acción */
        .action-icons a, .action-icons button,
        .document-card .card-actions a, .document-card .card-actions button {
            position: relative;
            overflow: hidden;
        }
        
        .action-icons a::after, .action-icons button::after,
        .document-card .card-actions a::after, .document-card .card-actions button::after {
            content: '';
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .document-card .card-actions a:focus:not(:active)::after, .document-card .card-actions button:focus:not(:active)::after {
        ...
        /* Estilos para el formato del documento */
        .format-badge {
        ...
        /* Estilo para el icono de documento en la tabla vacía */
        .empty-table-icon {
            font-size: 3rem;
            color: var(--primary-color);
            margin-bottom: 1rem;
...
<body>
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 mb-md-4">
                            <h2 class="card-title mb-3 mb-md-0">
                                <i class="bi bi-file-earmark-text me-2 d-none d-md-inline-block"></i>Gestión de Documentos
                            </h2>
                        </div>
                        ...
                        <div class="search-container mb-3 mb-md-4">
                            <form action="{{ route('documentos.index') }}" method="GET">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="search" placeholder="Buscar por nombre, descripción..." value="{{ request('search') }}">
                        ...
                        <!-- Tabla de documentos (vista desktop y tablet) -->
                                    ...
                                    @forelse($documentos as $documento)
                                        <tr>
                                            <td class="action-icons">
                                                <a href="{{ route('documentos.edit', $documento) }}" title="Editar" class="edit-icon">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <a href="{{ route('documentos.show', $documento) }}" title="Ver detalles" class="view-icon">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                                <button type="button" title="Eliminar" class="delete-icon" 
                                                    ...
                                                    data-nombre="{{ $documento->nombre == 'otro' && $documento->otro_nombre ? $documento->otro_nombre : $documento->nombre }}"
                                                    data-descripcion="{{ $documento->descripcion == 'otro' && $documento->otra_descripcion ? $documento->otra_descripcion : $documento->descripcion }}"
                                                    data-formato="{{ $documento->formato }}">
                                            ...
                                            <td>
                                                <span class="format-badge">{{ $documento->formato }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
    ...
    <a href="{{ route('documentos.create') }}" class="floating-action-button">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>¿Estás seguro de que deseas eliminar el documento <span class="document-info" id="documentName"></span>?</p>
                    <p class="mb-1">Descripción: <span class="document-info" id="documentDescription"></span></p>
                    <p>Formato: <span class="document-info" id="documentFormat"></span></p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (deleteModal) {
                deleteModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    document.getElementById('documentName').textContent = nombre;
                    document.getElementById('documentDescription').textContent = descripcion;
                    document.getElementById('documentFormat').textContent = formato;
                    
                    // Actualizar el formulario de eliminación
                    const deleteForm = document.getElementById('deleteForm');
                    if (deleteForm) {
                        deleteForm.action = `{{ route('documentos.destroy', '') }}/${id}`;
                    }
                });
