...
<head>
    ...
    <style>
        ...
        .recambio-card .card-header .nombre {
            font-weight: 600;
            font-size: 1.1rem;
            color: var(--primary-color);
        }
        
        .recambio-card .card-content {
            margin-bottom: 0.75rem;
        }
        
        .recambio-card .card-content .info-row {
            display: flex;
            justify-content: space-between;
            margin-bottom: 0.5rem;
        }
        
        .recambio-card .card-content .info-row .label {
            font-weight: 500;
            color: #666;
        }
        
        .recambio-card .card-content .info-row .value {
            font-weight: 400;
            text-align: right;
        }
        
        .recambio-card .card-footer {
            display: flex;
            justify-content: space-between;
            align-items: center;
            margin-top: 0.75rem;
        ...
        .recambio-card .card-actions {
            display: flex;
            gap: 8px;
        }
        
        .recambio-card .card-actions a, 
        .recambio-card .card-actions button {
            color: var(--primary-color);
            transition: var(--transition);
            text-decoration: none;
            display: inline-flex;
        ...
        .recambio-card .card-actions a.edit-icon, 
        .recambio-card .card-actions button.edit-icon {
            color: #0d6efd; /* Azul para editar */
        }
        
        .recambio-card .card-actions a.view-icon, 
        .recambio-card .card-actions button.view-icon {
            color: #198754; /* Verde para ver */
        }
        
        .recambio-card .card-actions a.delete-icon, 
        .recambio-card .card-actions button.delete-icon {
            color: #dc3545; /* Rojo para eliminar */
        }
        
        .recambio-card .card-actions a:hover, 
        .recambio-card .card-actions button:hover {
            background-color: rgba(79, 140, 255, 0.1);
        }
        
        .recambio-card .card-actions a.edit-icon:hover, 
        .recambio-card .card-actions button.edit-icon:hover {
            background-color: rgba(13, 110, 253, 0.1);
        }
        
        .recambio-card .card-actions a.view-icon:hover, 
        .recambio-card .card-actions button.view-icon:hover {
            background-color: rgba(25, 135, 84, 0.1);
        }
        
        .recambio-card .card-actions a.delete-icon:hover, 
        .recambio-card .card-actions button.delete-icon:hover {
            background-color: rgba(220, 53, 69, 0.1);
        }
        
        /* Animación de ripple para los botones de acción */
        .action-icons a, .action-icons button,
        .recambio-card .card-actions a, .recambio-card .card-actions button {
            position: relative;
            overflow: hidden;
        }
        
        .action-icons a::after, .action-icons button::after,
        .recambio-card .card-actions a::after, .recambio-card .card-actions button::after {
            content: '';
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .recambio-card .card-actions a:focus:not(:active)::after, .recambio-card .card-actions button:focus:not(:active)::after {
        ...
        /* Estilo para el icono de recambio en la tabla vacía */
        .empty-table-icon {
            font-size: 3rem;
            color: var(--primary-color);
            margin-bottom: 1rem;
...
<body>
                    ...
                    <div class="card-body">
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-3 mb-md-4">
                            <h2 class="card-title mb-3 mb-md-0">
                                <i class="bi bi-tools me-2 d-none d-md-inline-block"></i>Gestión de Recambios
                            </h2>
                        </div>
                        ...
                        <div class="search-container mb-3 mb-md-4">
                            <form action="{{ route('recambios.index') }}" method="GET">
                                <div class="input-group">
                                    <input type="text" class="form-control" name="search" placeholder="Buscar por producto, referencia..." value="{{ request('search') }}">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <strong>
                                            @if(request('search'))
                                                "{{ request('search') }}"
                                            @elseif(request('filter') == 'recientes')
                                                Recientes
                                            @endif
                                        </strong>
                                    </div>
                                    <a href="{{ route('recambios.index') }}" class="btn btn-sm btn-outline-secondary">
                        ...
                        <!-- Tabla de recambios (vista desktop y tablet) -->
                                    ...
                                    @forelse($recambios as $recambio)
                                        <tr>
                                            <td class="action-icons">
                                                <a href="{{ route('recambios.edit', $recambio) }}" title="Editar" class="edit-icon">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                <a href="{{ route('recambios.show', $recambio) }}" title="Ver detalles" class="view-icon">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                                <button type="button" title="Eliminar" class="delete-icon" 
                                                    ...
                                                    data-id="{{ $recambio->id }}" 
                                                    data-producto="{{ $recambio->producto }}">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="text-center py-4">
                                                ...
                                                @if(request('search'))
                                                    No se encontraron recambios que coincidan con los criterios de búsqueda.
                                                @else
                        ...
                        <div class="mobile-cards">
                            @forelse($recambios as $recambio)
                                <div class="recambio-card">
                                    <div class="card-header">
                                        <div class="nombre">{{ $recambio->producto }}</div>
                                    </div>
                                    <div class="card-content">
                                        <div class="info-row">
                                            <div class="label">Cantidad:</div>
                                            <div class="value">{{ $recambio->cantidad }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Referencia:</div>
                                            <div class="value">{{ $recambio->referencia ?? 'N/A' }}</div>
                                        </div>
                                        <div class="info-row">
                                            <div class="label">Precio:</div>
                                            <div class="value">{{ $recambio->precio ? number_format($recambio->precio, 2) . ' €' : 'N/A' }}</div>
                                        </div>
                                    </div>
                        ...
                        @if($recambios->hasPages())
                            <div class="d-flex justify-content-center mt-3 mt-md-4">
                                {{ $recambios->appends(['search' => request('search'), 'filter' => request('filter')])->links() }}
                            </div>
                        @endif
                    </div>
                </div>
    ...
    <a href="{{ route('recambios.create') }}" class="floating-action-button">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p>¿Estás seguro de que deseas eliminar el recambio <span class="recambio-info" id="recambioInfo"></span>?</p>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (deleteModal) {
                deleteModal.addEventListener('show.bs.modal', function(event) {
                    ...
                    document.getElementById('recambioInfo').textContent = producto;
                    
                    // Actualizar el formulario de eliminación
                    const deleteForm = document.getElementById('deleteForm');
                    if (deleteForm) {
                        deleteForm.action = `{{ route('recambios.destroy', '') }}/${id}`;
                    }
                });
