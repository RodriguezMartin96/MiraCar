            ...
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('recambios.update', $recambio) }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="siniestro_id" class="form-label">Siniestro (Opcional)</label>
                            <select class="form-select" id="siniestro_id" name="siniestro_id">
                                <option value="">Seleccionar siniestro</option>
                                @foreach($siniestros as $siniestro)
                                    <option value="{{ $siniestro->id }}" {{ old('siniestro_id', $recambio->siniestro_id) == $siniestro->id ? 'selected' : '' }}>
                                        {{ $siniestro->numero ?? $siniestro->id }} - {{ $siniestro->vehiculo->marca }} {{ $siniestro->vehiculo->modelo }} ({{ $siniestro->vehiculo->matricula }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
